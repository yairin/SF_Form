...
> **סטטוס נוכחי:** ה-API חובר בהצלחה ל-Sandbox האמיתי של סיילספורס ונבדק מול הסכימה האמיתית (ראו `API_IMPLEMENTATION_NOTES.md`) — אבל **טרם פרוס בכתובת נגישה לגורם החיצוני**, ועדיין חסר Id של Queue/User עבור `Case.OwnerId` (שדה חובה) בלעדיו כל יצירת קריאה תיכשל. הסעיף "מה עדיין חסר מהצד שלנו" בסוף המסמך מפרט את זה. אין לשלוח את המסמך כפי שהוא בלי להשלים את הפרטים המסומנים `<TBD>`.
...
2. **`Case.OwnerId` (שדה חובה) — אין לו ערך עדיין.** בלי Id של Queue/User אמיתי, כל קריאה ל-`POST /api/cases` תיכשל. ראו `API_IMPLEMENTATION_NOTES.md`.
