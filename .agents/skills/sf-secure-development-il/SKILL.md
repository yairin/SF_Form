...
## 0. בדיקת עדכניות סקילים לפני פיתוח — **חובה [DEV]**
לפני תחילת **כל** משימת פיתוח SF, לוודא שספריית הסקילים הרשמית עדכנית, ולרענן במידת הצורך:
```bash
npx skills add forcedotcom/sf-skills   # מרענן את .agents/skills (סימלינקים ב-.claude/skills)
```
- להריץ את הרענון בתחילת סשן פיתוח / כשעובדים על יכולת פלטפורמה חדשה (Flows, OmniStudio, Metadata API, Permission Sets, Experience וכו').
- לבחור ולהחיל את סקיל ה-`platform-*` / `automation-*` / `omnistudio-*` / `experience-*` הרלוונטי למשימה — הם מכילים את התחביר וה-API העדכניים (גרסת API, best practices). לא להסתמך על זיכרון בלבד.
- אם הרענון נכשל (רשת/מגבלה) — להמשיך עם הגרסה המותקנת ולציין זאת, ולרענן בהזדמנות.
- כלל זה חל יחד עם דרישות האבטחה שבהמשך: גם קוד מאובטח וגם עדכני-פלטפורמה.
